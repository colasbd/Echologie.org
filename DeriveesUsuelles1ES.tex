...
\usepackage{geometry}
\geometry{verbose,tmargin=22mm,bmargin=0cm,lmargin=23mm,rmargin=0cm}
...
node[draw, ellipse, fill, text=white, pos=.27] {\Large\bf Echologie.org}
arc (-90 : -180 : 4*\un)
-- ($(current page.north west)+(20*\un,-20*\un)$);

\un=1.2mm
\node(Triskell) at ($(current page.north west)+(16*\un,-14*\un)$){};
...
\parindent0cm \setlength{\extrarowheight}{13.6mm} \tabcolsep8mm 
...
\begin{tabular}{ccc|cr}
 ...
 \end{tikzpicture} \begin{tikzpicture}[{shift=(-.1,-2.7)}, overlay]
...
{\LARGE{}\begin{tikzpicture}[scale=.25,overlay]
		\draw[->,>=latex](-2,0) --(2,0);
		\draw[->,>=latex] (0,-2) -- (0,2);
		\draw plot[domain=-1.3:1.3](\x,\x^3*\x);
\end{tikzpicture}	} & {\LARGE{}$x^{4}$ } & {\LARGE{}$4x^{3}$ } & {\LARGE{}$u^{4}$ } & {\LARGE{}$4u^{3}\cdot u'$ }\tabularnewline
{\LARGE{}\begin{tikzpicture}[scale=.25,overlay]
		\draw[->,>=latex](-2,0) --(2,0);
		\draw[->,>=latex] (0,-2) -- (0,2);
...
{\LARGE{}\vphantom{{\LARGE{}$\frac{1}{\frac{1}{\frac{1}{\frac{1}{\frac{1}{1}}}}}$}}\begin{tikzpicture}[scale=.25,overlay]
		\draw[->,>=latex](-2,0) -- (2,0);
		\draw[->,>=latex](0,-2) -- (0,2);
