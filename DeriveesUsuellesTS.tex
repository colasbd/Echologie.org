...
\usepackage{geometry}
\geometry{verbose,tmargin=22mm,bmargin=0cm,lmargin=19mm,rmargin=0cm}
...
\parindent0cm \setlength{\extrarowheight}{8.1mm} \tabcolsep6mm 
...
{\LARGE{}\begin{tikzpicture}[scale=.2,overlay]
		\draw plot[smooth,domain=-5:5] (.5*\x,{sin(\x r)});
		\draw[->,>=latex] (-2.5,0) -- (2.5,0);
		\draw[->,>=latex] (0,-2.5) -- (0,2.5);
\end{tikzpicture}	} & {\LARGE{}$\sin x$ } & {\LARGE{}$\cos x$ } & {\LARGE{}$\sin u$ } & {\LARGE{}$\left(\cos u\right)\cdot u'$ }\tabularnewline
{\LARGE{}\begin{tikzpicture}[scale=.2,overlay]
		\draw plot[smooth,domain=-4:4] (.5*\x,{cos(\x r)});
		\draw[->,>=latex] (-2.5,0) -- (2.5,0);
		\draw[->,>=latex] (0,-2.5) -- (0,2.5);
\end{tikzpicture}	} & {\LARGE{}$\cos x$ } & {\LARGE{}$-\sin x$ } & {\LARGE{}$\cos u$ } & {\LARGE{}$-\left(\sin u\right)\cdot u'$ }\tabularnewline
{\LARGE{}\begin{tikzpicture}[scale=.2,overlay]
		\draw plot[smooth,domain=-1.2:1.2, samples=100] (.5*\x,{sin(\x r) / cos(\x r)});
		\draw plot[smooth,domain=-4.3:-1.95, samples=100] (.5*\x,{sin(\x r) / cos(\x r)});
		\draw plot[smooth,domain=1.95:4.3, samples=100] (.5*\x,{sin(\x r) / cos(\x r)});
		\draw[dashed,very thin] (pi / 4,-2.4) -- (pi / 4,2.6);
		\draw[dashed,very thin] (-pi / 4,-2.4) -- (-pi / 4,2.6);
		\draw[->,>=latex] (-2.5,0) -- (2.5,0);
		\draw[->,>=latex] (0,-2.5) -- (0,2.5);
\end{tikzpicture}	} & {\LARGE{}$\tan x$ } & {\LARGE{}${\displaystyle \frac{1}{\cos^{2}x}}$ } & {\LARGE{}$\tan u$ } & {\LARGE{}${\displaystyle \frac{1}{\cos^{2}u}\cdot u'}$ }\tabularnewline
{\LARGE{}\vphantom{{\LARGE{}$\frac{1}{\frac{1}{1}}$}} } & {\LARGE{}ou } & {\LARGE{}$1+\tan^{2}x$ } & {\LARGE{}ou } & {\LARGE{}$\left(1+\tan^{2}u\right)\cdot u'$ }\tabularnewline
\hline 
