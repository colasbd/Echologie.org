...
\usepackage[landscape,a4paper]{geometry}fr
...
\makeatother
\begin{document}
\title{\large\bf La Pyramide Sacrée}

{\footnotesize{}\setlength\extrarowheight{0mm}
\parindent0mm
\parskip0.6mm
...
\draw[->,>=latex] (a1) to[out=-10,in=10] node[left]{ 		\hsize4cm 		\vbox{ 			\begin{center} 				en remplaçant l'addition				 				par une soustraction 			\end{center} 		} 	} (a2);
\coeur[shift={($(60:0.5)+(0:1.3)+(0,.03)$)},ball color=lightgray,scale=0.05,rotate=-16,opacity=.2]
\node (a4) at ($(60:0.5)+(0:1.2)+(-50:0.25)$){$\displaystyle\frac{a+b}k = \frac ak+\frac bk$};
\draw[->,>=latex] (a3) to[out=-170,in=170] node[right]{ 		\hsize5cm 		\vbox{ 			\begin{center} 				en remplaçant la multiplication				 				par une division 			\end{center} 		} 	} (a4);
