...
\makeatother
\begin{document}
\title{\LARGE\bf Le lemme de Cesàro}

\textbf{\large{}Soit $(u_{n})_{n\in\mathbb{N}^{*}}$ une suite réelle
convergeant vers 0. On se propose de démontrer que la suite ${\displaystyle \left(\frac{\sum_{k=1}^{n}u_{k}}{n}\right)_{n\in\mathbb{N}^{*}}}$
converge également vers 0. Il s'agit donc de démontrer $\forall\varepsilon>0,\exists N\in\mathbb{N}^{*},\forall n\geq N,\left|\frac{\sum_{k=1}^{n}u_{k}}{n}\right|<\varepsilon$}{\large \par}
...
on sait qu'il existe $N_{1}\in\mathbb{N}^{*}$ vérifiant $\forall n\geq N_{1},|u_{n}|<\frac{\varepsilon}{2}$}{\large \par}
...
\textbf{\large{}}%
{\fboxsep 3mm\Ovalbox{\begin{minipage}[t]{1\columnwidth - 2\fboxsep - 1.6pt}%
\textbf{\large{}Soit donc $n\geq N_{1}$, on peut dès lors affirmer
:
\[
...
\bigskip{}
\textbf{\large{}De même, puisque ${\displaystyle \lim_{n\longrightarrow+\infty}\frac{\sum_{k=1}^{N_{1}-1}u_{k}}{n}=0}$,
on sait qu'il existe $N_{2}\in\mathbb{N}^{*}$ vérifiant $\forall n\geq N_{2},\left|\frac{\sum_{k=1}^{N_{1}-1}u_{k}}{n}\right|<\frac{\varepsilon}{2}$.}{\large \par}

\textbf{\large{}En posant $N=\max(N_{1},N_{2})$, on déduit de ce
qui précède que pour tout $n\geq N$ on a la majoration} :
